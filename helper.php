...
function regex($options = [], $c)
{
	...
	$valid['mode::smallest-selection-for*and+'] = 'U'; //ungreedy, * and + will match as little as possible, instead of as much as possible
...
// Containers
function oneOfTheseCharacters($c) 		    { return '[' . $c . ']'; }  // Char Class, inside everything is connected with or and no metChars work
function NegativoneOfTheseCharacters($c)   { return '[^' . $c . ']'; } // means at least one char (!) that is not inside the char class
function Group($c, $name = '') 	{ return '(' . (!empty($name) ? '?<' . $name . '>' : '') . $c . ')'; } // also with capturing
function NonCaptureGroup($c) 	{ return '(?:' . $c . ')'; }
...
// Positions (not 'consumptive', so no problems with multiple truns and preg_match_all())
